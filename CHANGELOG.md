...
- Isolated playground previews in an opaque-origin iframe sandbox and added nonce-based CSP, strict session and parent-origin validation, and restrictive browser policies to the embed response.
- Updated the Cloudflare, SvelteKit, and TypeScript ESLint toolchains and pinned SvelteKit's transitive `cookie` dependency to a patched release. The workspace dependency audit now reports no known vulnerabilities.

### Fixed

- Made current-value notifications deterministic under reentrant writes, subscription changes, and subscriber exceptions without leaving the store in a stuck flush state.
- Deduplicated frame scheduler dependency edges so equivalent `before` and `after` declarations no longer produce false cycles.
- Rejected invalid texture dimensions and options before allocation, including non-finite or fractional sizes and dimensions above the device limit.
- Fixed the Data Mosh playground demo in the opaque preview sandbox by requesting its video asset with CORS in all framework variants.
- Preserved physical resource identity in compute render graphs when logical aliases collide or share the same underlying GPU resource, while keeping disjoint texture subresources independent.
- Gave React `useFrame` registrations a stable task identity so dependency refs work before effects run, including registrations without an explicit key.
- Cleared queued storage-buffer writes after any failed frame so writes cannot leak into a later recovery frame.
...
- Compute shaders no longer receive every material storage buffer and storage texture implicitly. `ComputePass.resources` now declares the complete pass-local binding topology; an omitted map means no user resource bindings.
- Replaced `PingPongComputePass.target` and generated `${target}A`/`${target}B` WGSL names with required sampled-read and storage-write descriptors marked by `pingPong` roles.
- Removed `PingPongComputePass.getCurrentOutput()`, `getTarget()`, and `advanceFrame()`. The renderer publishes the latest ping-pong view under its logical material texture key.
- Consecutive compute passes now run in stable dependency order derived from resource reads and writes. Multiple writers and dependency cycles are errors instead of declaration-order behavior.

### Added

- Added explicit sampled texture, storage-write texture, storage-read buffer, storage-read-write buffer, and sampler descriptors with pass-local WGSL aliases.
- Added current/initial read versions, 2D texture subresource views, format-aware sampled WGSL types, typed fallback textures, sampler compatibility checks, and device-limit preflight.
...
- Compute pipelines are shared by shader and binding topology instead of material keys. Stable passes reuse layouts, bind groups, views, and graph plans without creating GPU objects per frame.
...
- Export `TextureOptionsInput` from the public Svelte and Vue adapter entrypoints.
...
[Unreleased]: https://github.com/Motion-Core/motion-gpu/compare/ad66dc54...HEAD
[0.15.1]: https://github.com/Motion-Core/motion-gpu/compare/5c8bfe5...ad66dc54
