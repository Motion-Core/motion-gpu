...
- LLM docs index: `https://spektral.madebyhex.com/llms.txt`
...
- Read the experimental, immutable graph snapshot with `useSpektral().graph.getSnapshot()` when tooling needs to inspect the current plan.
...
- Add a stable `label` when diagnostics or graph tooling must identify a pass. Unlabeled passes receive a deterministic `kind#declarationIndex` fallback in graph snapshots.
- Built-in fullscreen passes prepare active pipelines before the first frame. After `ShaderPass.setFragment(...)`, the last-known-good pipeline remains active until the newest source compiles.
- `useSpektral().graph.getSnapshot()` is experimental schema version 1. It returns a cached, deeply frozen snapshot until the graph plan changes. Node and resource IDs are opaque and stable only for the renderer lifetime; the snapshot never exposes GPU objects, WGSL, resource data, callbacks, or timing records.
...
- Compute passes are accepted in the same `passes` array as render passes and dispatch in stable resource-dependency order before the base scene render. Declaration order breaks ties between independent passes.
...
- `PingPongComputePass.resources` must contain exactly one sampled `pingPong: 'read'` descriptor and one storage-write `pingPong: 'write'` descriptor. Both reference the same material texture declared with `storage: true` and explicit `width`/`height`; their map keys become the pass-local WGSL aliases.
...
- Map error to fragment/include/define line. For `ShaderPass`, inspect `report.shader` for pass label, stage, input/output format, and whether the failure came from user or generated wrapper source.
- A failed hot edit is recoverable and keeps the last-known-good output. A failed first preparation blocks initial rendering.
