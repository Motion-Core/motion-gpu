...
async function startServer(): Promise<{ server: PreviewServer; url: string; outDir: string }> {
	const outDir = await mkdtemp(resolve(tmpdir(), 'motion-gpu-real-renderer-'));
...
async function runBrowser(page: Page): Promise<RealRendererBrowserResult> {
	return page.evaluate(async () => {
		const runner = window.__MOTION_GPU_REAL_RENDERER_BENCHMARK__;
		if (!runner) {
...
async function main(): Promise<void> {
	...
	if (args.updateBaseline) {
		...
		if (result.environment.powerMode !== 'ac-high-power') {
			throw new Error(
				`Refusing baseline update with powerMode=${result.environment.powerMode}; control the host and set MOTION_GPU_PERF_POWER_MODE=ac-high-power`
			);
		}
