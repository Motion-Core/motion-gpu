...
async function readStyleSignature(page: Page): Promise<OverlayStyleSignature> {
	return page.evaluate(() => {
		...
		const overlay = read('.spektral-error-overlay');
		const shell = read('.spektral-error-dialog-shell');
		const dialog = read('.spektral-error-dialog');
		const header = read('.spektral-error-header-top');
		const badges = read('.spektral-error-badges');
		const title = read('.spektral-error-title');
		const snippet = read('.spektral-error-source-snippet');
		const row = read('.spektral-error-source-row');
		const details = read('.spektral-error-details pre');
...
test.describe('shared error overlay styles', () => {
	test('preserves cascade and responsive layout', async ({ page }) => {
		...
		await expect(page.getByTestId('spektral-error')).toBeVisible();
		await page.evaluate(() => {
			document.documentElement.dataset['spektralCascadeProof'] = '';
		});
