...
export function attachShaderCompilationDiagnostics(
	...
	(error as SpektralErrorWithDiagnostics).spektralDiagnostics = freezeDiagnosticsPayload(payload);
	return error;
}
...
export function getShaderCompilationDiagnostics(
	...
	const payload = (error as SpektralErrorWithDiagnostics).spektralDiagnostics;
