...
import SpektralWithControlProbe from './fixtures/SpektralWithControlProbe.svelte';
import {
...
describe('FragCanvas runtime', () => {
	...
	it('rebuilds renderer when color pipeline changes', async () => {
		...
		expect(created[1]?.options.color).toEqual({ toneMapping: 'khronos-pbr-neutral' });
		expect(created[0]?.renderer.destroy).toHaveBeenCalledTimes(1);
	});
		...
		const view = render(SpektralWithControlProbe, {
			props: {
				onProbe: (value: unknown) => {
					contexts[0] = value as SpektralContext;
				},
				autoRender: true,
				dpr: 1,
				maxDelta: 0.1
