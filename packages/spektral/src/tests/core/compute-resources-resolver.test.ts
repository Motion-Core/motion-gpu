...
import { toSpektralErrorReport, type SpektralErrorCode } from '../../lib/core/error-report';
import type {
...
function diagnosticCode(run: () => unknown): SpektralErrorCode {
	try {
		run();
	} catch (error) {
		return toSpektralErrorReport(error, 'render').code;
	}
...
describe('resolveComputePassResources', () => {
	...
	it('emits stable diagnostic codes at resolver failure sites', () => {
		...
		expect(
			diagnosticCode(() =>
				normalizeComputeResourceMap({
					spektralFrame: { texture: 'camera', access: 'sampled' }
				})
			)
