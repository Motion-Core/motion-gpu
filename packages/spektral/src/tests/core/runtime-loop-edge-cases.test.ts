/**
 * Edge-case tests for createSpektralRuntimeLoop that are not covered by
...
function baseOptions(
	...
	extras: Partial<Parameters<typeof createSpektralRuntimeLoop>[0]> = {}
): Parameters<typeof createSpektralRuntimeLoop>[0] {
	return {
		canvas: createCanvas(),
		registry,
...
describe('runtime-loop edge cases', () => {
	...
	it('reports render error when writeStorageBuffer receives unknown buffer name', async () => {
		...
		const loop = createSpektralRuntimeLoop(baseOptions(registry, material, { reportError }));
	...
	it('rebuilds the renderer after the active device is lost', async () => {
		...
		const loop = createSpektralRuntimeLoop(baseOptions(registry, undefined, { reportError }));
