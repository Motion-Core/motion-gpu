...
import { useSpektral } from '../lib/vue/spektral-context.js';
import {
...
const SpektralProbe = defineComponent({
	name: 'VueSpektralProbe',
	props: {
		onProbe: {
			type: Function as PropType<(value: SpektralContext) => void>,
			required: true
		}
	},
	setup(props) {
...
const SpektralWithControlProbe = defineComponent({
	name: 'VueSpektralWithControlProbe',
	props: {
		onProbe: {
			type: Function as PropType<(value: SpektralContext) => void>,
			required: true
...
`
		...
		return () =>
			h(
				...
				{
					default: () => h(SpektralProbe, { onProbe: props.onProbe })
				}
			);
	}
});
...
const FragCanvasFrameMutationHarness = defineComponent({
	...
	props: {
		...
		onError: {
			type: Function as PropType<(report: SpektralErrorReport) => void>,
			required: false
		},
		onErrorHistory: {
			type: Function as PropType<(history: readonly SpektralErrorReport[]) => void>,
			required: false
		},
		errorHistoryLimit: {
			type: Number,
...
describe('Vue FragCanvas runtime', () => {
	...
	it('updates runtime context when control props change after mount', async () => {
		...
		const view = render(SpektralWithControlProbe, {
			props: {
				onProbe: (value: SpektralContext) => {
					contexts[0] = value;
				},
				autoRender: true,
				dpr: 1,
	...
	it('stops scheduling frames in manual mode while idle and wakes on advance()', async () => {
		...
		const onProbe = vi.fn();
		render(SpektralWithControlProbe, {
			props: {
				onProbe,
				renderMode: 'manual'
			}
