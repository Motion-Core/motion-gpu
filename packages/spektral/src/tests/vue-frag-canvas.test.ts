		...
		return () =>
			h(
				FragCanvas,
				{
					...
					onError: props.onError as ((report: SpektralErrorReport) => void) | undefined
				},
				{
					errorRenderer: ({ report }: { report: SpektralErrorReport }) =>
						h(
							'div',
...
describe('Vue FragCanvas', () => {
	...
	it('shows a readable error when WebGPU is unavailable', async () => {
		render(FragCanvas, {
			props: {
				...
				deviceDescriptor: { label: 'spektral-test-device' }
			}
		});

		await flushFrame(16);
		const error = await screen.findByTestId('spektral-error');
	...
	it('can disable the built-in error overlay while still reporting errors', async () => {
		...
		expect(screen.queryByTestId('spektral-error')).toBeNull();
	});
	...
	it('forwards native canvas attrs to the internal canvas', () => {
		...
		const wrapper = view.container.querySelector<HTMLElement>('.spektral-canvas-wrap');
